...
void main() {
  ...
  test(
    'ICP-A: job-seeker is steered to a work-viable city, not a beach',
    () async {
      ...
      final plan = await generator.generate(
        answers: const [
          ...
          Answer(
            ...
            values: ['job_opportunities', 'low_cost'],
          ),
  ...
  test(
    'ICP-A: low-signal (balanced) user is nudged toward a practical city',
    () async {
      final generator = MigrationPlanGenerator(
        citiesRepository: _FakeCitiesRepository(cities),
      );
