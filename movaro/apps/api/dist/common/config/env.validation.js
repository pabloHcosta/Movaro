...
function validateEnvironment(config) {
    const nodeEnv = config.NODE_ENV ?? environment_enum_1.AppEnvironment.Development;
    const allowedEnvironments = Object.values(environment_enum_1.AppEnvironment);
    if (!allowedEnvironments.includes(nodeEnv)) {
        throw new Error(`Invalid NODE_ENV "${nodeEnv}". Expected one of: ${allowedEnvironments.join(', ')}`);
    }
    ...
    validatePositiveInteger(config.OUTBOUND_REQUEST_TIMEOUT_MS, 'OUTBOUND_REQUEST_TIMEOUT_MS');
...
function validateSupabaseConfig(config) {
    ...
    if (Boolean(supabaseUrl) != Boolean(effectiveSecret)) {
        throw new Error('SUPABASE_URL and SUPABASE_SECRET_KEY (or SUPABASE_SERVICE_ROLE_KEY) must be provided together.');
    }
    if (!supabaseUrl) {
        return;
    }
