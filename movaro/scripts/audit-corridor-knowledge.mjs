...
for (const entry of catalog.entries ?? []) {
  ...
  if (!entry.authority || !entry.title) errors.push(`${entry.id}: missing attribution`);
  if (!String(entry.url).startsWith('https://')) errors.push(`${entry.id}: URL must use HTTPS`);
  const verified = new Date(`${entry.verifiedAt}T00:00:00Z`);
  if (Number.isNaN(verified.getTime())) errors.push(`${entry.id}: invalid verifiedAt`);
  ...
  if (ageDays > maxDays) errors.push(`${entry.id}: source review is stale (${ageDays} days)`);
}
...
for (const relative of claimsFiles) {
  ...
  for (const claim of forbiddenClaims) {
    if (claim.test(body)) errors.push(`${relative}: forbidden universal claim ${claim}`);
  }
}
